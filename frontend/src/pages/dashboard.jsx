...
const Dashboard = () => {
 ...
 const handleDownloadDocument = async (e, doc) => {
  ...
  const res = await fetch(
    `${import.meta.env.VITE_API_URL}/documents/${doc.doc_id}`,
    {
      headers: {
  ...
  console.log(res);
  const data = await res.json();
  const fileName = `${data.title || "document"}.${getExtension(data.language)}`;
      ...
      <div className="sidebar">
        <div className="sidebarTop">
          ...
          <form onSubmit={handleJoinWorkspace} className="joinForm">
            <input value={joinWsId} onChange={(e) => setJoinWsId(e.target.value)} placeholder="Enter the workspace ID" />
      ...
      style={{
        ...
        <div className="dashboardHeader">
          <div className="headerLeft">
            ...
            <h2 className="mainHead">{activeWorkspace ? activeWorkspace.name : "Select Workspace"}</h2>
            {activeWorkspace && (
          ...
          {activeWorkspace && (
            <button className="primaryBtn" onClick={() => setShowDocModal(true)}>
              + New File
            </button>
          )}
        </div>
